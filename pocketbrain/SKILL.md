...
- **Header icon alignment**: `web_ui.css` must style `.view-header h1` with `display:inline-flex;align-items:center;gap:10px` and `svg { display:block; flex-shrink:0 }`, otherwise the Heroicon may appear misaligned or not render at all in the accessibility tree.
