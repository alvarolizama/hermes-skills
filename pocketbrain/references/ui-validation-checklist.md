...
- `#tab=type_entity`, `#tab=type_concept`, `#tab=type_comparison`, `#tab=type_query`, `#tab=type_raw` — summaries render markdown, not escaped text.
...
| Graph tab active but blank | `renderProjectGraph` expects `d.rems` but `collectProjectData` returns `d.reminders` | use fallbacks: `const reminders = d.reminders \|\| d.rems \|\| []` |
| Type view summaries show raw `**markdown**` | summary passed through `esc()` instead of `mdToHtml()` | render with `mdToHtml` and bind markdown links |
